...
lizards %>% 
  count(spp) %>% 
  arrange(-n)      # arrange in decreasing order of n
```
...
utst <- lizards %>% 
  ...
  filter(tail %in% c("B","W"))     # could do all in the same line
```
...
```{r}
utst %>% 
  count(site, tail)  # long format table, doesn't look great because two rows of "EAST", ... better for data analysis but not presentation or chi-squared test function

tail_counts <- utst %>% 
  janitor::tabyl(site, tail)   # better for presentation or chi-squared test function

# And you can get a table with both counts & proportions! 
tail_proportions <- tail_counts %>%            # could have piped directly into this
  janitor::adorn_percentages() %>%      # chi-squared is there a significant difference between proportions
  janitor::adorn_pct_formatting(digits = 2) %>% 
  janitor::adorn_ns()  # then can use KableExtra and rownames
```
...
lizard_ct <- tail_counts %>% 
  column_to_rownames(var = "site")   # sort of changes variable to observation
```
...
```{r}
lizard_x2 <- chisq.test(lizard_ct)   # from R, df = (#rows -1)*(#col-1) (parameter term)
lizard_x2    #  there is significant correlation effect on site and tail breakage - non-independence

lizard_tidy <- tidy(lizard_x2)  # table 
```
